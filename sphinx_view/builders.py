...
def process_django_views(app, env, *args):
    ...
    for module, code_snippets in env.django_code_snippets.items():
        with open(Path(app.outdir) / f"{pymodule(module)}.py", "w") as fh:
            for view_name, python in code_snippets.items():
                fh.write(f"# This code is generated by django-sphinx-view using `{module}.rst`\n"
                         f"# !!! DO NOT EDIT !!!\n# .. django-view:: {view_name}\n\n")
                fh.write(python)
                fh.write("\n\n\n")
    context = {
        "modules": [pymodule(module) for module in env.django_code_snippets.keys()],
        "urlpatterns": {pymodule(module): pattern for module, pattern in env.django_urlpatterns.items()},
    }
